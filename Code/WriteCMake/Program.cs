...
namespace WriteCMake
{
    class Program
    {
        ...
        class CProjFile
        {
            ...
            public string GetCMakeContentAsApplication(string sLinkWith)
            {
                if (!m_bExist)
                {
                    return "";
            ...
            public string GetCMakeContentAsCudaLib()
            {
                if (!m_bExist)
                {
                    return "";
...
set_target_properties({0} PROPERTIES CUDA_SEPARABLE_COMPILATION ON)", m_sName);
        ...
        static string MakeCMake(EArch eArch)
        {
            ...
            sContent += "MESSAGE(\"52 for 970, 61 for GTX10, 70 for V100, 75 for RTX20, 86 for RTX30\")\n";
        ...
        static void Main(string[] args)
        {
            //======================================
            //Config here
            string sLibName = "CNTensors";
            string[] sApplicationName = {"CNTest"};
            EArch eDefaultArch = EArch.EArcSM61;

            //======================================
            CProjFile projBase = new CProjFile(sLibName);

            string sContent = MakeCMake(eDefaultArch);
            sContent += projBase.GetCMakeContentAsCudaLib();

            foreach (string sProj in sApplicationName)
            {
                CProjFile projApp = new CProjFile(sProj);
                sContent += projApp.GetCMakeContentAsApplication(sLibName);
            }
