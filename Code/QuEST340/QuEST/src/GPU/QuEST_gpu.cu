...
void densmatr_initPureState(Qureg targetQureg, Qureg copyQureg)
{
    ...
    CUDABlocks = Ceil(copyQureg.numAmpsPerChunk, threadsPerCUDABlock);
...
void densmatr_initPlusState(Qureg qureg)
{
    ...
    int threadsPerCUDABlock, CUDABlocks;
    threadsPerCUDABlock = 128;
    CUDABlocks = Ceil(qureg.numAmpsPerChunk, threadsPerCUDABlock);
...
{   
    ...
    cudaMalloc(&(qureg->firstLevelReduction), static_cast<size_t>(ceil(qureg->numAmpsPerChunk/(qreal)REDUCE_SHARED_SIZE)*sizeof(qreal)));
    cudaMalloc(&(qureg->secondLevelReduction), static_cast<size_t>(ceil(qureg->numAmpsPerChunk/(qreal)(REDUCE_SHARED_SIZE*REDUCE_SHARED_SIZE)))*
            sizeof(qreal));
...
int statevec_initStateFromSingleFile(Qureg *qureg, char filename[200], QuESTEnv env){
    ...
    while (fgets(line, sizeof(char)*200, fp) != NULL && totalIndex<stateVecSize){
        if (line[0]!='#'){
            int chunkId = static_cast<int>(totalIndex/chunkSize);
...
int statevec_compareStates(Qureg mq1, Qureg mq2, qreal precision){
    qreal diff;
    int chunkSize = static_cast<int>(mq1.numAmpsPerChunk);
...
void statevec_compactUnitary(Qureg qureg, int targetQubit, Complex alpha, Complex beta) 
{
    int threadsPerCUDABlock, CUDABlocks;
    threadsPerCUDABlock = 128;
    CUDABlocks = Ceil(qureg.numAmpsPerChunk>>1, threadsPerCUDABlock);
...
void statevec_multiControlledMultiQubitUnitary(Qureg qureg, long long int ctrlMask, int* targs, int numTargs, ComplexMatrixN u)
{
    ...
    int CUDABlocks = Ceil(qureg.numAmpsPerChunk>>numTargs, threadsPerCUDABlock);
...
{
    int threadsPerCUDABlock = 128;
    int CUDABlocks = Ceil(qureg.numAmpsPerChunk>>2, threadsPerCUDABlock); // one kernel eval for every 4 amplitudes
...
void statevec_multiControlledUnitary(
    ...
    int threadsPerCUDABlock = 128;
    int CUDABlocks = Ceil(qureg.numAmpsPerChunk>>1, threadsPerCUDABlock);
...
void statevec_swapQubitAmps(Qureg qureg, int qb1, int qb2) 
{
    int threadsPerCUDABlock, CUDABlocks;
    threadsPerCUDABlock = 128;
    CUDABlocks = Ceil(qureg.numAmpsPerChunk>>2, threadsPerCUDABlock);
...
void statevec_multiControlledMultiQubitNot(Qureg qureg, int ctrlMask, int targMask) {
    ...
    int numThreadsPerBlock = 128;
    int numBlocks = Ceil(qureg.numAmpsPerChunk, numThreadsPerBlock);
...
qreal densmatr_findProbabilityOfZero(Qureg qureg, int measureQubit)
{
    ...
    while (numValuesToReduce > 1) {
        ...
        if (numValuesToReduce < maxReducedPerLevel) {
            valuesPerCUDABlock = static_cast<int>(numValuesToReduce);
            numCUDABlocks = 1;
        }
        // otherwise use only full CUDA-BLOCKS
        else {
            valuesPerCUDABlock = maxReducedPerLevel; // constrained by shared memory
            numCUDABlocks = Ceil(numValuesToReduce, valuesPerCUDABlock);
        }
...
void densmatr_calcProbOfAllOutcomes(qreal* outcomeProbs, Qureg qureg, int* qubits, int numQubits) {
    ...
    int numBlocks = Ceil(numDiags, numThreadsPerBlock);
...
void densmatr_collapseToKnownProbOutcome(Qureg qureg, int measureQubit, int outcome, qreal outcomeProb) {
    ...
    int threadsPerCUDABlock, CUDABlocks;
    threadsPerCUDABlock = 128;
    CUDABlocks = Ceil(numBasesToVisit, threadsPerCUDABlock);
...
void densmatr_mixDensityMatrix(Qureg combineQureg, qreal otherProb, Qureg otherQureg) {
    ...
    int threadsPerCUDABlock, CUDABlocks;
    threadsPerCUDABlock = 128;
    CUDABlocks = Ceil(numAmpsToVisit, threadsPerCUDABlock);
...
__global__ void statevec_applyPhaseFuncOverridesKernel(
 ...
 void statevec_applyPhaseFuncOverrides(
    ...
    // call kernel
    int threadsPerCUDABlock = 128;
    int CUDABlocks = Ceil(qureg.numAmpsPerChunk, threadsPerCUDABlock);
